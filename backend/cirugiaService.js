...
async function agendarCirugiaAtomica(
  ...
  try {
    ...
    const resPabellon = await client.query(
      "SELECT estado FROM Pabellones WHERE id = $1 FOR UPDATE",
      [pabellonId],
    );
    ...
    const insertQuery = `
        ...
        `;
    const resCirugia = await client.query(insertQuery, [
      rutPaciente,
      pabellonId,
      camaId,
      tipoCirugia,
    ...
    await client.query(
      `UPDATE Pabellones SET estado = 'Reservado' WHERE id = $1`,
      [pabellonId],
    );
    await client.query(`UPDATE Camas SET estado = 'Reservado' WHERE id = $1`, [
      camaId,
    ]);
