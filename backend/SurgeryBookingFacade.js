...
const { LaboratorioExternoAPI, RecursosHumanosExternaAPI, InventarioExternoAPI } = require('../shared/api/ExternalServicesApi');
...
class GestorCirugiasFacade {
    ...
    async validarYAgendarCirugia(payload) {
        ...
        console.log(`Paciente: ${payload.pacienteId} | Médico: ${payload.medicoId} | Tipo: ${payload.tipoCirugia}`);
        ...
        try {
            ...
            if (resultado.aprobado) {
                console.log('✅ Todas las validaciones pasaron — Cirugía habilitada para agendamiento');
                return {
                    exito: true,
                ...
                const criticasFallidas = fallidas.filter(r => r.severidad === 'CRITICA');

                // Mensaje especial para fatiga médica (CA-01 del SRS)
                const hayFatiga = fallidas.some(r => r.regla === 'Fatiga Médica (Horas de Turno)');
                ...
                console.log(`❌ Validación rechazada — ${fallidas.length} regla(s) fallida(s)`);
                return {
                    exito: false,
    ...
    async _construirContexto(payload) {
        ...
        console.log(`  Lab: apto=${resultadoLab.aptoParaCirugia} | RRHH: horas=${estadoMedico.horasSemanalesAcumuladas} | Inventario: ok=${estadoInventario.disponible}`);
        ...
        const horasTurnoContinuo = estadoMedico.horasSemanalesAcumuladas > 44
        ...
        return {
            ...
            insumos:   estadoInventario.disponible
                ? (payload.insumos ?? 15)
                : 0                          // inventario no disponible → fuerza fallo
        };
    }
}
