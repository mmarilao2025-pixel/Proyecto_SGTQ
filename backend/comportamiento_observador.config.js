...
const CONFIG_OBSERVER = {
  ...
  eventos: {
    ...
    criticos: ["emergencia_medica", "sistema_caido", "insumo_agotado"],
  },
...
function validarConfiguracion() {
  ...
  if (eventosCriticosInvalidos.length > 0) {
    throw new Error(
      `Eventos críticos no habilitados: ${eventosCriticosInvalidos.join(", ")}`,
    );
  }
  ...
  if (config.rendimiento.timeout_notificacion < 1000) {
    console.warn(
      "⚠️ Timeout de notificación muy bajo, puede causar problemas de rendimiento",
    );
  }

  if (config.logging.max_registros_historial > 10000) {
    console.warn(
      "⚠️ Máximo de registros de historial muy alto, puede afectar memoria",
    );
  }
