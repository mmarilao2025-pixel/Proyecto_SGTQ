...
function validarConfiguracion() {
  ...
  const reglasSinPrioridad = Object.keys(config.prioridades).filter(
    (regla) =>
      !config.prioridades[regla] ||
      config.prioridades[regla] < 1 ||
      config.prioridades[regla] > 5,
  );

  if (reglasSinPrioridad.length > 0) {
    throw new Error(
      `Reglas sin prioridad válida: ${reglasSinPrioridad.join(", ")}`,
    );
  }
