        ...
        <div className="space-y-4">
          ...
          <div className="rounded-2xl bg-slate-50 p-4">
            ...
            <p className="font-semibold">
              {stats.ultimoEvento?.tipo || "Sin eventos recientes"}
            </p>
            <p className="text-xs text-slate-400">
              {stats.ultimoEvento?.timestamp || ""}
            </p>
          </div>
          <div>
            <p className="text-xs uppercase tracking-[0.12em] text-slate-400 mb-2">
              Tipos de eventos
            </p>
            <div className="grid grid-cols-2 gap-2">
              {Object.entries(stats.tiposEventos).map(([tipo, count]) => (
                <div
                  key={tipo}
                  className="rounded-2xl bg-white p-3 border border-slate-100"
                >
