...
// Validación de RUT (Módulo 11 impecable)
...
const PatientRegistry: React.FC<PatientRegistryProps> = ({
  ...
  const [pacienteEncontrado, setPacienteEncontrado] = useState<Paciente | null>(
    null,
  );
  ...
  // 2. Estado del formulario expandido con valores por defecto consistentes
  ...
  const buscarPaciente = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        setError(
          "El RUT no está registrado. Complete el formulario para crear la ficha clínica.",
        );
  ...
  const registrarPaciente = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        setError(
          errorData.error || "Error al registrar el paciente en el sistema.",
        );
      }
    } catch (err) {
    ...
    <div className="bg-white rounded-2xl shadow-sm border border-slate-200 p-6 w-full mx-auto">
      <div className="border-b border-slate-100 pb-4 mb-6">
        <h2 className="text-xl font-bold text-slate-800 flex items-center">
          <i className="fa-solid fa-folder-open text-blue-600 mr-3"></i> 1.
          Validación de Ficha Clínica
        </h2>
        <p className="text-sm text-slate-500 mt-1">
          Busque al paciente o registre uno nuevo antes de asignar el quirófano.
        </p>
      </div>
      ...
      <div className="flex flex-col md:flex-row gap-4 mb-8 bg-slate-50 p-4 rounded-xl border border-slate-200">
        <form onSubmit={buscarPaciente} className="flex flex-1 gap-2">
          ...
          <button
            type="submit"
            disabled={loading}
            className="bg-slate-800 text-white px-6 py-2.5 rounded-lg font-semibold hover:bg-slate-700 transition shadow-sm"
          >
        ...
        >
          <i className="fa-solid fa-user-plus mr-2"></i> Registrar Nuevo
          Paciente
        </button>
      </div>

      {error && (
        <div className="mb-6 p-4 bg-amber-50 text-amber-800 text-sm rounded-lg border border-amber-200">
          {error}
        </div>
      )}
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
            ...
            <div>
              ...
              <p className="font-medium text-slate-700">
                {pacienteEncontrado.rut}
              </p>
            </div>
          </div>
          <div className="mt-6 flex justify-end">
            <button
              onClick={() => onPacienteValidado(pacienteEncontrado)}
              className="bg-blue-600 text-white px-8 py-3 rounded-xl font-bold shadow-md hover:bg-blue-700 transition"
            >
              Avanzar a Quirófano{" "}
      ...
      {modoRegistro && (
        ...
        <form onSubmit={registrarPaciente} className="animate-fade-in space-y-6 bg-white p-6 rounded-xl border border-emerald-100 shadow-sm">
          ...
          <div className="pt-4">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              {/* Alergias Multi-selección con Texto Libre */}
              ...
              {/* Enfermedades Crónicas Multi-selección */}
              <div className="block md:col-span-2">
        ...
        <form
          ...
          className="animate-fade-in space-y-5 bg-white p-6 rounded-xl border border-emerald-100 shadow-sm"
        >
          <h3 className="font-bold text-emerald-800 border-b border-emerald-100 pb-2 mb-4">
            Ficha de Nuevo Paciente
          </h3>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
            <label className="block">
              ...
              <input
                ...
                className="mt-1 block w-full rounded-lg border-slate-300 bg-slate-50 p-2.5 text-sm outline-none"
              />
            </label>
            <label className="block">
              <span className="text-xs font-semibold text-slate-600">
